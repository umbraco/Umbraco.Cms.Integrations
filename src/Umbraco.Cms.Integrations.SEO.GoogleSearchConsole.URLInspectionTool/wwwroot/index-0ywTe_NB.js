...
function Q({
  ...
  return { stream: async function* () {
    ...
    for (; !w.aborted; ) {
      ...
      l !== void 0 && k.set("Last-Event-ID", l);
      try {
        const z = {
          redirect: "follow",
          ...d,
        ...
        let x = new Request(m, z);
        r && (x = await r(m, z));
        const p = await (d.fetch ?? globalThis.fetch)(x);
        ...
        w.addEventListener("abort", I);
        try {
          for (; ; ) {
            const { done: V, value: _ } = await S.read();
            if (V) break;
            y += _, y = y.replace(/\r\n?/g, `
`);
            const A = y.split(`
...
`);
              ...
              $ && (i && await i(C), s && (C = await s(C))), e == null || e({
                data: C,
                event: q,
                id: l,
                retry: f
          ...
          w.removeEventListener("abort", I), S.releaseLock();
        }
        break;
      } catch (z) {
        if (t == null || t(z), n !== void 0 && h >= n)
          break;
        const x = Math.min(f * 2 ** (h - 1), a ?? 3e4);
        await g(x);
      }
    }
  }() };
...
const X = (r) => {
  ...
  const c = X(s), n = i.map((a) => s === "label" || s === "simple" ? r ? a : encodeURIComponent(a) : T({
    allowReserved: r,
    name: e,
    value: a
  })).join(c);
  ...
  if (s)
    for (const i of s) {
      ...
      if (o == null)
        continue;
      if (Array.isArray(o)) {
        e = e.replace(i, D({ explode: c, name: n, style: a, value: o }));
        continue;
      }
      if (typeof o == "object") {
        e = e.replace(
...
const R = (r) => te({
  ...
  var s;
  const e = { ...r, ...t };
  return (s = e.baseUrl) != null && s.endsWith("/") && (e.baseUrl = e.baseUrl.substring(0, e.baseUrl.length - 1)), e.headers = L(r.headers, t.headers), e;
}, oe = (r) => {
  const t = [];
...
const ie = () => ({
  ...
  let t = N(H(), r);
  const e = () => ({ ...t }), s = (d) => (t = N(t, d), e()), i = ie(), c = async (d) => {
    const l = {
      ...t,
      ...d,
      fetch: d.fetch ?? t.fetch ?? globalThis.fetch,
      headers: L(t.headers, d.headers),
...
}, fe = de(H({ baseUrl: "https://localhost:44370/", throwOnError: !0 })), ue = {
  type: "localization",
...
}, he = ue, pe = [
  {
    ...
    element: () => import("./googlesearchconsole-workspace.element-ULFLDrfD.js"),
    weight: 30,
    meta: {
...
//# sourceMappingURL=index-0ywTe_NB.js.map
