    ...
    public class JavascriptResponse
    {
        ...
        protected JavascriptResponse(bool success, string data, string error)
        {
            ...
            ScriptBuilder.AppendLine(success
                ? "window.opener.postMessage({ type: 'google:oauth:success', url: location.href, code: '" + data + "' }, '*');"
                : "window.opener.postMessage({ type: 'google:oauth:error', url: location.href, response: '" + error + "' }, '*');");
