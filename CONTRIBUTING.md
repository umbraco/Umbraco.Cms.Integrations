...
> **Which branch?** This guide describes the **v17 line** (`main-v17` / `v17/dev`).
...
| Branch | Purpose |
|---|---|
| `v17/dev` | Working branch. All changes land here first. |
| `main-v17` | Reflects the **last released** state. Never commit working changes here. |
| `v17/feature/*` | Feature branches, cut from and merged back to `v17/dev`. |
| `v17/release/*` | Cut from `v17/dev` per release; builds the publishable artifacts. |
| `v17/hotfix/*` | Cut from `main-v17` for an urgent fix on the released state. |

The flow is: work on a feature branch → PR into `v17/dev` → cut a release branch
from `v17/dev` → publish from there → merge the release branch back into both
`main-v17` and `v17/dev`.
...
Use `examples/Umbraco.Cms.Integrations.Testsite.V17` to run the packages in a real
Umbraco instance.
...
> `tests/` and `examples/Umbraco.Cms.Integrations.Testsite.V17/` each have their own
...
{
    "version": "7.0.1",
    "publicReleaseRefSpec": ["^refs/heads/main-v17$"]
    // ...
}
```
...
On `main-v17` a build produces a clean version (`7.0.2`). On any other branch NBGV
...
**A release is built from a release branch.** `version.json` lists `main-v17`,
`v17/release/*` and `v17/hotfix/*` as public release refs, and NBGV only drops the
...
So the release branch is what produces the artifact you publish, and `main-v17` /
`v17/dev` stay untouched until the release is proven. This matches how
`Umbraco.Automate` and `Umbraco.AI` work.

Release branches are named `v<N>/release/YYYY.MM.N` (calendar-based, e.g.
`v17/release/2026.08.1`). That is independent of the package versions — one branch
...
> its `version.json` differs from `main-v17` — the bump *is* the declaration. There
...
- **`/release-management`** — detects changed packages, recommends the bump per the
  ...
  entry, and pushes.
- **`/post-release-cleanup`** — merges the release branch into `main-v17` and back
  into `v17/dev`, bumps the patch on dev so the next dev build sorts above the
  release, and offers to delete the branch.

Full sequence:

1. `/release-management` on `v17/dev` — cuts `v17/release/YYYY.MM.N`, bumps,
...
>
> The branch is `v17/release/<date>` while tags are `release/<slug>-<version>`. They
